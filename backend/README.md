...
- 데이터는 named volume(`postgres-data`, `mongo-data`)에 보존됩니다.

`spring-boot-docker-compose` 지원이 포함되어 있어, 앱을 실행하면 프로젝트 루트의
`compose.yaml` 에 정의된 **PostgreSQL·MongoDB 컨테이너가 자동으로 기동**되고
접속 정보(datasource URL / mongo URI / 계정)가 앱에 자동 주입됩니다.
