...
import com.doq.comfozi.structuring.detection.ExceptionFlag
 ...
 */
data class RecordStructured(
    val ingestionId: Long,
    val recordId: Long,
    // 원본 근거(SourceRef)용 — 파일 출처면 채워지고 수기면 null
    val uploadType: IngestionUploadType?,
    val rowNo: Int?,
    // 관찰값(시스템 구조화) — TODO: 실제 매핑/정규화 결과로 교체 (현재는 raw)
    val observed: Map<String, String?>,
    // 예외 플래그 — TODO: detection 결과로 교체 (현재는 빈)
    val flags: Set<ExceptionFlag>,
)
