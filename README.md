...
**키가 없어도 PDF 는 처리됩니다.** 읽어낸 원문을 한 행의 품목명 자리에 담고 나머지는 비워 두므로,
필수값 누락으로 검수 인박스에 올라오고 사람이 화면에서 보고 채울 수 있습니다. 없는 값을 지어내지
않으면서 파일이 아무 데도 안 나타나는 일도 막는 절충입니다. 필수 흐름(CSV/XLSX + 수기 입력)은
키와 무관하게 전부 동작합니다.
...
LLM 추출기는 `ANTHROPIC_API_KEY` 가 있을 때만 켜진다. **없으면 읽어낸 원문을 한 행에 담아** 나머지
필드는 비운 채 검수로 올린다 — 사람이 화면에서 보완하는 경로다. 이미지(PNG·JPEG)는 촬영본이라
